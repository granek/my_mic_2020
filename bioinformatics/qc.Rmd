...
We could use code like this to load the data and store it in a Seurat object.
...
```{r}
head(tem@meta.data)
tem[["percent.mt"]] <- PercentageFeatureSet(tem, pattern = "^mt-")
head(tem@meta.data)
```

Now, do the same thing but for all seurat objects in our list
```{r}
add_mt <- function(so){
...
#saveRDS(seulist.filt, file = file.path(intermeddir, "seulist-filt.rds"))
